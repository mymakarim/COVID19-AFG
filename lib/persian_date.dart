...
class PersianDate {
  ...
  List<String> _monthShort = const <String>[
    // shor name
    'حَمَل',
    'ثور',
    'جوزا',
  ...
  List<String> _monthLong = const <String>[
    'حَمَل',
    'ثور',
    'جوزا',
    'سَرَطان',
  ...
  _toFormat(String format) {
    ...
    if (newFormat.indexOf(MM) != -1)
      newFormat = newFormat.replaceFirst(MM, _monthLong[this.month - 1]);
    if (newFormat.indexOf(M) != -1)
      newFormat = newFormat.replaceFirst(M, _monthShort[this.month - 1]);
    ...
    if (newFormat.indexOf(DD) != -1)
      newFormat = newFormat.replaceFirst(DD, _dayLong[this.weekday - 1]);
    if (newFormat.indexOf(D) != -1)
      newFormat = newFormat.replaceFirst(D, _dayShort[this.weekday - 1]);
  ...
  DateTime jalaliToGregorian(String parseDate) {
    var re = _parseFormat;
    Match match = re.firstMatch(parseDate);
